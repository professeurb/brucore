...
let pop t = if t.len = 0 then raise Empty else begin
  t.len <- t.len - 1 ;
  let v = t.car.(t.len) in
  if Array.length t.car >= 2 && t.len <= Array.length t.car / 4 then begin
    let new_car = Array.make (Array.length t.car / 2) (Obj.magic 0) in
    for i = 0 to t.len - 1 do
      new_car.(i) <- t.car.(i)
    done ;
    t.car <- new_car
...
let pop_opt t = if t.len = 0 then None else begin
  t.len <- t.len - 1 ;
  let v = t.car.(t.len) in
  if Array.length t.car >= 2 && t.len <= Array.length t.car / 4 then begin
    let new_car = Array.make (Array.length t.car / 2) (Obj.magic 0) in
...
let iter f t =
  ...
  for i = 0 to t.len - 1 do f carrier.(i) done ;;
